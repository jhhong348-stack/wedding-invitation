...
/* ========================================
   LANGUAGE SELECTOR — BASIC SYSTEM
======================================== */
...
function setWeddingLanguage(
  ...
  if (
    ...
    typeof weddingTranslations === "undefined"
  ) {
    return;
  }

  const languageData =
    weddingTranslations[language];
...
languageToggleButton?.addEventListener(
  ...
  () => {
    const isOpen =
      languageToggleButton.getAttribute(
        "aria-expanded"
      ) === "true";
...
/*
  이전에 선택한 언어가 있으면 복원합니다.
  최초 방문자는 한국어로 표시합니다.
*/
const savedWeddingLanguage =
  readSavedLanguage();

setWeddingLanguage(
  savedWeddingLanguage || "ko",
  {
    save: false
  }
);